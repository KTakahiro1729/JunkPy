...
class TestIfEval(TestJunkTypeEval, unittest.TestCase):
    source = 'a = False\nif a:\n    print(1)\n    print(2)\nelse:\n    print(2)\n'
    actual_stdin = ''
    expect_code = '[None for ns in[__builtins__ if(type(__builtins__) is dict)else{attr:getattr(__builtins__,attr)for attr in dir(__builtins__)}]if[ns.update({"a":False})]if[[None for ns in[ns]if[ns["print"](1)]if[ns["print"](2)]][0]if(ns["a"])else[None for ns in[ns]if[ns["print"](2)]][0]]][0]'
    expect_stdout = '2\n'
    expect_value = None
